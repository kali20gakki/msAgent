...
class ChatApplicationService:
    ...
    _HOTKEY_COMMANDS = frozenset({"/hotkeys"})
    _TOOL_COMMANDS = frozenset({"/tools"})
    _EXIT_COMMANDS = frozenset({"/exit"})
    _CLEAR_COMMANDS = frozenset({"/clear"})
    _NEW_SESSION_COMMANDS = frozenset({"/new"})
    _BACKEND_STATUS_COMMANDS = frozenset(
        {"/backend", "/backend status", "/shell", "/shell status"}
    )
    _BACKEND_SWITCH_MAP = {
    ...
    _COMMAND_HELP: tuple[tuple[str, str], ...] = (
        ...
        ("/new", "开启新会话并清空上下文"),
        ("/clear", "清空当前会话历史"),
        ("/backend status", "显示当前 deepagents backend"),
        ("/backend filesystem", "切换到 FilesystemBackend"),
        ("/backend local_shell", "切换到 LocalShellBackend（高风险）"),
        ("/shell on", "快捷开启 LocalShellBackend"),
        ("/shell off", "快捷关闭 LocalShellBackend"),
        ("/exit", "退出 msAgent"),
    )
    ...
    def get_status_message(self) -> str:
        ...
        lines = [f"Current deepagents backend: {backend_name} ({backend_mode})."]
        if backend_mode == "local_shell":
            lines.append(
                "`execute` will run shell commands directly on this machine without sandboxing."
            )
        else:
            lines.append("Host shell execution is currently disabled.")
        lines.append(
            "Commands: /backend filesystem | /backend local_shell | /shell on | /shell off"
        )
        return "\n".join(lines)
    ...
    def resolve_user_input(self, raw_input: str) -> UserIntent:
        ...
        if normalized in self._HELP_COMMANDS:
            return UserIntent("help")
        if normalized in self._HOTKEY_COMMANDS:
            return UserIntent("hotkeys")
        if normalized in self._TOOL_COMMANDS:
            return UserIntent("tools")
    ...
    def get_available_tools(self) -> list[dict[str, object]]:
        return self._backend.get_available_tools()
    ...
    def find_commands(self, query: str, limit: int = 8) -> list[tuple[str, str]]:
        ...
        for index, (command, description) in enumerate(self._COMMAND_HELP):
            ...
            if normalized_command == normalized_query:
                ...
                continue
            scored.append((score, index, command, description))
