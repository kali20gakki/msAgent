...
def _resolve_tool_command(command: str) -> str:
    ...
    if resolved:
        return resolved
    raise RuntimeError(f"{command} is required to run the official deep-agents-ui frontend")
...
def ensure_ui_standalone_checkout() -> Path:
    ...
    if not extract_bundled_ui_standalone(checkout_dir):
        raise RuntimeError("Bundled standalone web UI is not available in this installation.")
    return checkout_dir
...
def _find_extracted_ui_root(extract_dir: Path, *, marker_path: Path) -> Path:
    ...
    if len(marker_candidates) == 1:
        return (
            marker_candidates[0].parent
            if marker_path.name == str(marker_path)
            else marker_candidates[0].parents[len(marker_path.parts) - 1]
        )

    raise RuntimeError(f"Bundled deep-agents-ui archive did not contain {marker_path.as_posix()}")
