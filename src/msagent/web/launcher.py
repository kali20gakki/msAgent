...
def _print_startup_status(
    *,
    ...
    console.print(f"API URL: [bold]{api_url}[/bold] | Graph ID: [bold]{LANGGRAPH_GRAPH_ID}[/bold]")
    console.print(f"Working dir: {working_dir.resolve()}")
    if start_ui:
        console.print(f"On first open, set Deployment URL to {api_url} and Assistant ID to {LANGGRAPH_GRAPH_ID}.")
    else:
...
async def _wait_for_http_service(
    *,
    ...
    while True:
        if process.poll() is not None:
            raise RuntimeError(f"{service_name} failed to start. Check the logs above for the underlying error.")
        ...
        if asyncio.get_running_loop().time() >= deadline:
            raise RuntimeError(f"Timed out waiting for {service_name} on {target_host}:{port}.")
