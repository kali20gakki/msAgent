...
class NetworkConfig(BaseModel):
    ...
    local: list[str] = Field(default_factory=list, description="Allowed local unix sockets")
...
class SandboxConfig(VersionedConfig):
    ...
    def validate_current_os(self) -> None:
        ...
        if self.os != current_os:
            raise RuntimeError(f"Sandbox '{self.name}' requires {self.os.value}, but running on {current_os.value}")


class SandboxProfileBinding(BaseModel):
    """Binds patterns to a sandbox profile."""

    sandbox: SandboxConfig | None = Field(default=None, description="Sandbox config (None for no-sandbox)")
...
class AgentSandboxConfig(BaseModel):
    ...
    profiles: list[SandboxProfileBinding] = Field(default_factory=list, description="Profile bindings")
