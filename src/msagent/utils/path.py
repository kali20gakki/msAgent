...
def is_windows_absolute_path(path: str) -> bool:
    ...
    return bool(_WINDOWS_DRIVE_ABSOLUTE_RE.match(path) or _WINDOWS_UNC_ABSOLUTE_RE.match(path))


def is_absolute_path_like(path: str) -> bool:
    """Check whether a path should be treated as absolute across platforms."""
    expanded = Path(path).expanduser()
    return expanded.is_absolute() or path.startswith("/") or is_windows_absolute_path(path)
...
def resolve_path(working_dir: str, path: str) -> Path:
    ...
    if not is_absolute:
        ...
        if original.exists() and is_symlink_escape(original, [working_path]):
            raise SymlinkEscapeError(f"Symlink escapes working directory: {path} -> {resolved}")
...
def expand_pattern(pattern: str, working_dir: Path, include_nonexistent: bool = False) -> list[Path]:
...
def matches_hidden(path: str | Path, hidden_patterns: list[str], working_dir: Path) -> bool:
