    ...
    def validator(v: Any) -> Any:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Failed to parse JSON (auto-repair failed): {e}") from e

        if not isinstance(parsed, list):
            raise TypeError(f"Expected JSON array for {model_cls.__name__}, got {type(parsed).__name__}: {parsed}")
            ...
            if args and inspect.isclass(args[0]):
                try:
                    if issubclass(args[0], BaseModel):
                        ...
                        validators[f"_parse_{param_name}"] = field_validator(param_name, mode="before")(
